...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.sleepUntil(
      ...
      new Date(new Date().getTime() + 20 * 60 * 60 * 1000),
    );
...
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{ cron: "0 0 6 * * *" }] }, //06:00 UTC = 8:00 AM EGY
  ...
  async ({ step }) => {
    ...
    const { startUTC, endUTC } = await step.run("get-today-date", () => {
      ...
      const startOfDay = new Date(egyptTime);
      startOfDay.setHours(0, 0, 0, 0);

      const endOfDay = new Date(egyptTime);
    ...
    if (absentEmployees.length > 0) {
      await step.run("send-reminder-emails", async () => {
        const emailPromises = absentEmployees.map((emp) => {
          ...
          <br/>
          <P style="font-size:16px;">Best Regards,</p>
          <P style="font-size:16px;"><strong>AldeebEMS</strong></p>
        </div> `,
          });
        });
